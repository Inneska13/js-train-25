...
async function getDataFromUrl(URL) {
  try {
    ...
    if (!response.ok) {
      throw new Error("Винекла помилка під час отримання даний");
    }
...
async function postDataWithAuth(url, data, authToken) {
  try {
    ...
    if (!response.ok) {
      throw new Error("Винекла помилка під час отримання даний", error);
    }
...
while (!countdown.next().done) {
  const currentValue = countdown.next().value;
